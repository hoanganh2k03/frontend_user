    ...
    const fetchProducts = async ({ page, limit, category, subCategory, search }) => {
        try {
          ...
          if (category && category.length > 0) {
            const categoryIds = [];
            if (category.includes('Men')) categoryIds.push(2);
            if (category.includes('Women')) categoryIds.push(3);
            if (category.includes('Kids')) categoryIds.push(4);
          ...
          const response = await fetch(
            `${backendUrl}/api/product/filterProducts?page=${page}&limit=${limit}${
